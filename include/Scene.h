...
public:
	...
	~Scene();
	//virtual void init(std::string name, float windowWidth, float windowHeight);
	void baseInit(std::string name, std::string fileName, float windowWidth, float windowHeight);
	virtual void init();
	std::string getName();
	void addGameObjectToScene(GameObject* object);
	void addInputToScene(Input* input);
	GameObject* getGameObject(std::string name);
	void baseUpdate(Display* display);
