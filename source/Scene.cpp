...
Scene::Scene(std::string name, std::string fileName, float windowWidth, float windowHeight)
{
	...
	for (int i = 0; i < level->getWidth(); i++)
	{
		for (int j = 0; j < level->getHeight(); j++)
		{
			...
			addGameObjectToScene(GameObject::createSquarePartTexture(name + "_Floor_" + std::to_string(i) + "_" + std::to_string(j), SPOT_WIDTH, 0, SPOT_DEPTH, false, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
			...
			//Ceiling
			texX = level->getPixel(i, j).r / NUM_TEXTURES; //16 textures in the collection
			texY = texX % NUM_TEX_EXP;
			texX /= NUM_TEX_EXP;

			addGameObjectToScene(GameObject::createSquarePartTexture(name + "_Ceiling_" + std::to_string(i) + "_" + std::to_string(j), SPOT_WIDTH, 0, SPOT_DEPTH, true, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
			...
			texX = level->getPixel(i, j).b / NUM_TEXTURES; //16 textures in the collection
			texY = texX % NUM_TEX_EXP;
			texX /= NUM_TEX_EXP;

			if (level->getPixel(i, j - 1).r == 0 && level->getPixel(i, j - 1).g == 0 && level->getPixel(i, j - 1).b == 0)
			{
				addGameObjectToScene(GameObject::createCubePartTexture(name + "_Wall_a_" + std::to_string(i) + "_" + std::to_string(j), SPOT_WIDTH, SPOT_HEIGHT, 0, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
			...
			if (level->getPixel(i, j + 1).r == 0 && level->getPixel(i, j + 1).g == 0 && level->getPixel(i, j + 1).b == 0)
			{
				addGameObjectToScene(GameObject::createCubePartTexture(name + "_Wall_b_" + std::to_string(i) + "_" + std::to_string(j), SPOT_WIDTH, SPOT_HEIGHT, 0, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
			...
			{
				addGameObjectToScene(GameObject::createCubePartTexture(name + "_Wall_c_" + std::to_string(i) + "_" + std::to_string(j), 0, SPOT_HEIGHT, SPOT_DEPTH, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
			...
			if (level->getPixel(i + 1, j).r == 0 && level->getPixel(i + 1, j).g == 0 && level->getPixel(i + 1, j).b == 0)
			{
				addGameObjectToScene(GameObject::createCubePartTexture(name + "_Wall_d_" + std::to_string(i) + "_" + std::to_string(j), 0, SPOT_HEIGHT, SPOT_DEPTH, (1.0f - (float)texX / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), 1.0f - (float)texX / (float)NUM_TEX_EXP, 1.0f - (float)texY / (float)NUM_TEX_EXP, (1.0f - (float)texY / (float)NUM_TEX_EXP) - (1.0f / (float)NUM_TEX_EXP), TextureManager::getTextureManager()->getTexture("Wolf"), ShaderManager::getShaderManager()->getShader("Phong")));
